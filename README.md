...
Here's how to modify your `InstrumentProcessor` to process instruments concurrently with maximum performance while maintaining safety:
...
export class InstrumentProcessor {
  ...
  constructor(
    ...
    ruleEngineWrapper: any,
    concurrencyLimit: number = 10 // Default to 10 concurrent processes
  ) {
    this.ruleHandlerChain = ruleHandlerChain;
    this.ruleEngineWrapper = ruleEngineWrapper;
  ...
  async processInstruments(instruments: any[]): Promise<any[]> {
    const processedInstruments: any[] = [];
    const pendingPromises: Set<Promise<void>> = new Set();

    for (const instrument of instruments) {
      // Create a processing promise for each instrument
      const processingPromise = this.processInstrument(instrument)
        .then(result => {
          if (result) {
            processedInstruments.push(result);
          }
          pendingPromises.delete(processingPromise);
        });

      pendingPromises.add(processingPromise);

      // When we hit the concurrency limit, wait for at least one to finish
      if (pendingPromises.size >= this.concurrencyLimit) {
  ...
  private async processInstrument(instrument: any): Promise<any> {
    try {
      const result = await this.ruleHandlerChain.handle(instrument);
      return result;
    } finally {
      // Ensure reset happens even if handling fails
      this.ruleEngineWrapper.reset();
    }
  }
}
...
1. **Concurrent Processing**:
   - Processes instruments up to the concurrency limit simultaneously
   - Uses `Promise.race` to manage the concurrency pool

2. **Thread Safety**:
   - Each instrument gets its own processing promise
   - The rule engine wrapper is reset after each instrument (in a finally block)

3. **Configurable Concurrency**:
   - Defaults to 10 concurrent processes
   - Can be adjusted based on your system resources

4. **Error Handling**:
   - Maintains error handling from the original implementation
   - Uses finally block to ensure cleanup

### Usage Example:

```typescript
// Create processor with higher concurrency for bulk processing
const processor = new InstrumentProcessor(
  ruleHandlerChain,
  ruleEngineWrapper,
  20 // Process up to 20 instruments concurrently
);

// Process large batch of instruments
const results = await processor.processInstruments(largeInstrumentArray);
```

### Advanced Version with Error Collection:

If you want to track failed instruments:

```typescript
async processInstruments(instruments: any[]): Promise<{
  successes: any[];
  failures: { instrument: any; error: Error }[];
}> {
  const results = {
    successes: [] as any[],
    failures: [] as { instrument: any; error: Error }[]
  };

  // ... same concurrency logic, but modify the processing:
  ...
  const processingPromise = this.processInstrument(instrument)
    ...
    .catch(error => {
      results.failures.push({ instrument, error });
    })
    .finally(() => {
      pendingPromises.delete(processingPromise);
    });
...
- Maximum throughput while preventing resource exhaustion
